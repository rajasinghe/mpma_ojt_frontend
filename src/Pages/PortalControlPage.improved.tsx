...
import api from "../api";
import { Modal, Button, Badge, Dropdown, Card, Row, Col } from "react-bootstrap";
...
export default function PortalControlPage() {
  const [traineesWithoutPortalAccounts, setTraineesWithoutPortalAccounts] = useState<any[]>([]);
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    const traineesWithEmail = filteredTrainees.filter((t) => t.email);
    if (e.target.checked) {
  ...
  const onSubmitCreateAccount = async (data: CreateAccountFormData) => {
    ...
    try {
      ...
      await Swal.fire({
        title: "Error!",
        text: error.response?.data?.message || "Failed to create portal account",
        icon: "error",
        confirmButtonText: "OK",
      });
    } finally {
          ...
          <Row className="align-items-center">
            <Col md={6}>
              <div className="position-relative">
                <i className="bi bi-search position-absolute"
                   style={{ left: '16px', top: '50%', transform: 'translateY(-50%)', color: '#6c757d' }}></i>
                <input
                  type="text"
            ...
            <Col md={6}>
              <div className="d-flex justify-content-end align-items-center gap-3">
                {searchRegistered && (
                  <Badge bg="info" style={styles.statusBadge}>
              ...
              <Col xs="auto">
                <Dropdown>
                  ...
                  <Dropdown.Menu>
                    ...
                    <Dropdown.Item onClick={() => handleSort('NIC_NO')}>
                      NIC {sortField === 'NIC_NO' && (sortDirection === 'asc' ? '↑' : '↓')}
                    </Dropdown.Item>
                    <Dropdown.Item onClick={() => handleSort('institute_name')}>
                      Institute {sortField === 'institute_name' && (sortDirection === 'asc' ? '↑' : '↓')}
                    </Dropdown.Item>
                    <Dropdown.Item onClick={() => handleSort('start_date')}>
              ...
              <table className="table table-hover mb-0">
                ...
                <tbody>
                  {filteredTrainees.map((trainee, idx) => (
                    <tr key={idx} className={!trainee.email ? 'table-warning' : ''}>
                      <td>
                        {trainee.email ? (
                          <input
                            type="checkbox"
                      ...
                      <td>
                        <code className="text-dark">{trainee.NIC_NO}</code>
                      </td>
                      <td>
                        <div className="d-flex align-items-center">
                          ...
                          <span className="fw-medium">{trainee.name}</span>
                        </div>
                      </td>
                      <td>
                        {trainee.email ? (
                          <span className="text-muted">{trainee.email}</span>
                        ) : (
                          <Badge bg="warning" text="dark">No Email</Badge>
                        )}
                      </td>
                      <td>
                      ...
                      <td>
                        <small className="text-muted">
                          {moment(trainee.start_date).format("MMM DD, YYYY")}
                        </small>
                      </td>
                      <td>
